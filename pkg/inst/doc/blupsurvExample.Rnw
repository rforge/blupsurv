...
<<quickfit,eval=FALSE>>=
vigndata.quickfit <- bivrec( Surv2(start,stop,pain,fever) ~ age + sex + cluster(state) + id(patientID), data=vigndata, K1=10, K2=10, verbose=0)
@
...
vigndata.quickfit2 <- bivrec(Surv2(start,stop,pain,fever) ~ age + sex + cluster(state) + id(patientID), K1=10, K2=10, excludevars1="age", data=vigndata, verbose=0)
@
<<quickfit2sum>>=
...
<<finalfit,eval=FALSE>>=
vigndata.fit <- bivrec(Surv2(start,stop,pain,fever) ~ age + sex + cluster(state) + id(patientID), K1=1, K2=1, excludevars1="age", data=vigndata, verbose=0)
@
<<finalfitsum>>=
...
plot(vigndata.fit$frailty$subj1,vigndata.fit$frailty$subj2,xlab="pain",ylab="fever",main="Subject-level frailties",pch=19, asp=1,xlim=c(0,3),ylim=c(0,3) )
@
\begin{figure}
\begin{center}
